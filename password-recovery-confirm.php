...
if ($mysql->connect_errno > 0) {
    ...
    $sql = "SELECT userId, activated, email from success_uploaded_users JOIN user_login_details ON userId = user_login_details.`id` JOIN user_personal_info ON userId = user_personal_info.id AND email = '$mail";
    if ($run = $mysql->query($sql)) {
        if ($run->num_rows == 1) {
